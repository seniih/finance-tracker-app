...
class PurchaseSaleForm extends StatefulWidget {
  final bool isSale;

  const PurchaseSaleForm({super.key, required this.isSale});

  @override
  State<PurchaseSaleForm> createState() => _PurchaseSaleFormState();
}

class _PurchaseSaleFormState extends State<PurchaseSaleForm> {
  bool _isSaving = false;
  final _formKey = GlobalKey<FormState>();
  final _draft = _PurchaseSaleDraft();
  ...
  List<String> get _currentContactList {
    ...
    return _allContacts.where((c) {
      ...
      if (_draft.transactionType != null && _draft.transactionType!.isNotEmpty && c.currency != _draft.transactionType) return false;
      return true;
    }).map((c) => c.name).toList();
  }

  List<String> get _currentSafeList {
    if (_draft.transactionType == null || _draft.transactionType!.isEmpty) {
      return _safes.map((s) => s.name).toList();
    }
    return _safes.where((s) => s.currency == _draft.transactionType).map((s) => s.name).toList();
  }
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text(
                  widget.isSale ? 'Yeni Satış Kaydı' : 'Yeni Alış Kaydı',
            ...
            FormRow(children: [
              buildAutocomplete(
                ...
                (val) {
                  setState(() {
                    _draft.mainAccount = val;
                    try {
                      final safe = _safes.firstWhere((s) => s.name == val);
                      _draft.transactionType = safe.currency;
                    } catch (_) {}
                  });
                },
              ),
              buildAutocomplete(
                'Para Birimi',
                _draft.transactionType,
                _typeOptions,
                (val) {
                  setState(() {
                    _draft.transactionType = val;
                    // TL seçilince kur karşılığı sıfırla
                    if (val == 'TL') _draft.exchangeRate = null;
                  });
                },
              ),
            ]),
            const SizedBox(height: AppSpacing.xxl),
            // TL dışındaki para birimlerinde kur karşılığı göster
            if (_draft.transactionType != null && _draft.transactionType != 'TL')
              Padding(
                padding: const EdgeInsets.only(bottom: AppSpacing.xxl),
                child: FormRow(children: [
                  buildTextField('Tutar', (val) => _draft.amount = val, isNumber: true),
                  buildTextField('O Anki Kur Karşılığı (TL)', (val) => _draft.exchangeRate = val, isNumber: true),
                ]),
              )
            else
              Padding(
                padding: const EdgeInsets.only(bottom: AppSpacing.xxl),
                child: buildTextField('Tutar', (val) => _draft.amount = val, isNumber: true),
              ),

            // Bölüm 2: Kar Merkezi
            _SectionHeader(title: '2. Kategori', icon: Icons.category_outlined),
            const SizedBox(height: AppSpacing.lg),
            FormRow(children: [
              buildAutocomplete(
                'Kar Merkezi',
            ...
            FormRow(children: [
              ...
              const SizedBox(), // Boşluk doldurmak için
            ]),
            const SizedBox(height: AppSpacing.xxxl),
            ...
            Align(
              ...
              child: _isSaving
                  ...
                  : ElevatedButton.icon(
                      ...
                      icon: Icon(widget.isSale ? Icons.sell_outlined : Icons.shopping_cart_outlined),
                      label: Text(
                        widget.isSale ? 'Satışı Kaydet' : 'Alışı Kaydet',
  ...
  Future<void> _saveForm() async {
    ...
    try {
      final safe = _safes.firstWhere((s) => s.name == _draft.mainAccount);
      if (safe.currency != _draft.transactionType) {
    ...
    if (_draft.contactName != null) {
      try {
        final contact = _allContacts.firstWhere((c) => c.name == _draft.contactName);
        if (contact.currency != _draft.transactionType) {
    ...
    final transactionData = {
      ...
      'main_account_id': _draft.mainAccount != null ? _safes.firstWhere((s) => s.name == _draft.mainAccount).id : null,
      'currency': _draft.transactionType,
      'amount': double.tryParse(_draft.amount ?? '0') ?? 0,
      'exchange_rate': double.tryParse(_draft.exchangeRate ?? '0') ?? 0,
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${widget.isSale ? "Satış" : "Alış"} işlemi başarıyla kaydedildi!'),
      ...
      setState(() {
        ...
        _draft.mainAccount = null;
        _draft.transactionType = null;
        _draft.amount = null;
        _draft.exchangeRate = null;
